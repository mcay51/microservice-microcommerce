...
public class HealthCheckController {
    ...
    @Autowired
    public HealthCheckController(HealthEndpoint healthEndpoint, RestTemplate restTemplate) {
    ...
    public ResponseEntity<Map<String, Object>> getHealthStatus() {
        ...
        healthDetails.put("service", "order-service");
        
        // Check dependent services
        Map<String, Object> dependencies = new HashMap<>();
        dependencies.put("product-service", checkProductServiceHealth());
        healthDetails.put("dependencies", dependencies);
        ...
        try {
            ...
            if (!productServiceAccessible) {
                return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                        .body(Health.down()
                                .withDetail("message", "Order service is not ready - Product service is not accessible")
                                .build());
            }
            
            return ResponseEntity.ok(Health.up()
                    .withDetail("message", "Order service is ready to accept traffic")
                    .withDetail("dependencies", Map.of("product-service", "UP"))
                    .build());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(Health.down()
                            .withDetail("error", e.getMessage())
                            .withDetail("message", "Order service is not ready")
                            .build());
        }
    }
    ...
    public ResponseEntity<Health> getLiveness() {
        try {
            ...
            return ResponseEntity.ok(Health.up()
                    .withDetail("message", "Order service is alive")
                    .build());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(Health.down()
                            .withDetail("error", e.getMessage())
                            .withDetail("message", "Order service is not alive")
                            .build());
        }
    }
