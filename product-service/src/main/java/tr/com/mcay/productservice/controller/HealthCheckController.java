...
public class HealthCheckController {
    ...
    public ResponseEntity<Map<String, Object>> getHealthStatus() {
        ...
        healthDetails.put("service", "product-service");
    ...
    public ResponseEntity<Health> getReadiness() {
        try {
            ...
            return ResponseEntity.ok(Health.up()
                    .withDetail("message", "Product service is ready to accept traffic")
                    .build());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(Health.down()
                            .withDetail("error", e.getMessage())
                            .withDetail("message", "Product service is not ready")
                            .build());
        }
    }
    ...
    public ResponseEntity<Health> getLiveness() {
        try {
            ...
            return ResponseEntity.ok(Health.up()
                    .withDetail("message", "Product service is alive")
                    .build());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(Health.down()
                            .withDetail("error", e.getMessage())
                            .withDetail("message", "Product service is not alive")
                            .build());
        }
    }
} 